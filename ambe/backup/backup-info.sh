...
#NameUser=$(curl -sS https://${Server_URLL}/registered.txt | grep $MYIP | awk '{print $2}')
export NameUser=$( curl -s https://${Server_URLL}/registered.txt | grep -w $Your_License_Key | cut -d ' ' -f 9-100 | awk '{print $1}' );
...
data=$(curl -sS https://raw.githubusercontent.com/jaka1m/backup-full/main/$NameUser/$NameUser-last-backup)
echo
